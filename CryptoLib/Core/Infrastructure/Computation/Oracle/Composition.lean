import CryptoLib.Core.Infrastructure.Computation.Oracle.Bounds
...
theorem totalCost_le_separated
  ...
  | query name oracleQuery =>
      simp only [runExact] at hresult
      rw [PMF.mem_support_bind_iff] at hresult
      rcases hresult with ⟨issueResult, _hissueResult, horacleResult⟩
      rw [PMF.mem_support_bind_iff] at horacleResult
      rcases horacleResult with ⟨oracleResult, _horacleResult, hresult⟩
      rw [PMF.mem_support_pure_iff] at hresult
      subst result
      exact le_refl (issueResult.cost + oracleResult.cost)
...
theorem oracleCost_le_totalQueries_nsmul
  ...
  | query name oracleQuery =>
      ...
      rcases hresult with ⟨issueResult, _hissueResult, horacleResult⟩
      rw [PMF.mem_support_bind_iff] at horacleResult
      rcases horacleResult with ⟨oracleResult, horacleResult, hresult⟩
      rw [PMF.mem_support_pure_iff] at hresult
      subst result

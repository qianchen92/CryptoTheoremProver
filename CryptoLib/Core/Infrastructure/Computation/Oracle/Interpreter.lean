import CryptoLib.Core.Infrastructure.Computation.Oracle.Handler
import CryptoLib.Core.Infrastructure.Computation.Oracle.Program
import CryptoLib.Core.Infrastructure.Computation.Oracle.Trace
...
theorem bind_runExact_erase
    {β : Type uObserved}
    (program : Program issueAlgebra α) (sec : CryptoLib.Core.SecPar)
    (env : CostedOracleEnv.{uCost, uOracle, uQuery, uResponse, uState} M Spec)
    (state : env.State)

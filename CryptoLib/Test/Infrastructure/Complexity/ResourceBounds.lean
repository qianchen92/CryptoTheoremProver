...
namespace CryptoLib.Test.Infrastructure.Complexity.ResourceBounds
...
noncomputable def oneStepOperationalModel :
    OperationalModel
      ...
      (CryptoLib.Core.SecPar → Nat) where
  Code := Unit
  ...
  fail_if_success
    have _machine :
      ...
      { toTimedMachine := oneStepMachine
        runtime_poly :=
          CryptoLib.Core.Infrastructure.Asymptotic.IsPolyBounded.const 1 }
  trivial
...
end CryptoLib.Test.Infrastructure.Complexity.ResourceBounds

...
structure ComponentCostCertificate
  ...
  initBudget : CryptoLib.Core.SecPar → Address → M.Cost
  activationBudget : CryptoLib.Core.SecPar → Address → M.Cost
  erasureBudget : CryptoLib.Core.SecPar → Address → M.Cost
  leakageBudget : CryptoLib.Core.SecPar → Address → M.Cost
...
structure StepCostCertificate
    ...
    (network : (sec : CryptoLib.Core.SecPar) → NetworkAdapter family sec) where
  component : ComponentCostCertificate family
  atomBudget : CryptoLib.Core.SecPar → M.Cost
  ...
  kernelBudget_le : ∀ sec (primitive : KernelPrimitive) (addresses : List Address),
    M.instPartialOrder.le (cost primitive addresses) (atomBudget sec)

namespace StepCostCertificate

variable {cost : KernelCost M Address}
variable {network : (sec : CryptoLib.Core.SecPar) → NetworkAdapter family sec}
...
def budget
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) : M.Cost :=
...
private theorem pure_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) (count : Nat) (value : Value) :
...
private theorem bind_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) {First : Type uBoundFirst} {Next : Type uBoundNext}
...
private theorem pad_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) {dist : RandCosted M Value}
    {used available : Nat}
...
private theorem init_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) (address : Address) :
...
private theorem leakage_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) (address : Address) (state : family.State sec address) :
...
private theorem charge_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) (primitive : KernelPrimitive) (addresses : List Address) :
    RandCosted.CostBound (KernelCost.charge cost primitive addresses)
      (repeatActivationCost M 1 (certificate.atomBudget sec)) := by
  letI := M.instAddMonoid
  letI := M.instPartialOrder
  intro result hresult
  simp only [KernelCost.charge, RandCosted.liftCosted,
    PMF.mem_support_pure_iff] at hresult
  subst result
  calc
    cost primitive addresses ≤ certificate.atomBudget sec :=
      certificate.kernelBudget_le sec primitive addresses
    _ = repeatActivationCost M 1 (certificate.atomBudget sec) := by
      letI := M.instAddMonoid
      exact (one_nsmul _).symm
...
private theorem withCharge_sound
    ...
    (nextBound : RandCosted.CostBound next
      (repeatActivationCost M nextCount (certificate.atomBudget sec))) :
    RandCosted.CostBound
      (KernelCost.withCharge cost primitive addresses next)
      (repeatActivationCost M (nextCount + 1) (certificate.atomBudget sec)) := by
  unfold KernelCost.withCharge
...
private theorem enqueueDirect_sound
    (certificate : StepCostCertificate cost network)
    (sec : CryptoLib.Core.SecPar) (configuration : Configuration family policy sec)
    (activation : QueuedActivation schema) :
    RandCosted.CostBound
      (Kernel.enqueueDirect cost (network sec) configuration activation)
      (repeatActivationCost M 1 (certificate.atomBudget sec)) := by
...
private theorem routeEmissionCore_sound
    ...
    RandCosted.CostBound
      (Kernel.routeEmissionCore cost (network sec) configuration emission)
      (repeatActivationCost M 1 (certificate.atomBudget sec)) := by
...
private theorem routeEmission_sound
    ...
    {source : Address} (emission : Emission schema source) :
    RandCosted.CostBound
      (Kernel.routeEmission cost (network sec) configuration emission)
...
private theorem routeEmissionAs_sound
    ...
    RandCosted.CostBound
      (Kernel.routeEmissionAs cost (network sec) configuration controller
...
private theorem processAction_sound
    ...
    RandCosted.CostBound
      (Kernel.processAction cost (network sec) configuration source honest state action)
...
private theorem corruptFromState_sound
    ...
    RandCosted.CostBound
      (Kernel.corruptFromState cost (network sec) configuration source target
...
private theorem processCorruption_sound
    ...
    RandCosted.CostBound
      (Kernel.processCorruption cost (network sec) configuration source target)
...
private theorem activateHonest_sound
    ...
    RandCosted.CostBound
      (Kernel.activateHonest cost (network sec) configuration activation honest)
...
theorem step_sound
    ...
    (sec : CryptoLib.Core.SecPar) (configuration : Configuration family policy sec) :
    RandCosted.CostBound
      (Kernel.stepOne cost (network sec) configuration) (certificate.budget sec) := by
...
theorem runCosted_sound
    ...
    ∀ (fuel : Nat) (sec : CryptoLib.Core.SecPar)
      ...
      RandCosted.CostBound
        (Kernel.runCosted cost (network sec) fuel configuration)
...
structure FuelCertificate
    ...
    (fuel : CryptoLib.Core.SecPar → Nat) where
  noTimeout : ∀ sec result, result ∈
      (Kernel.runCosted cost (network sec) (fuel sec) (initial sec)).support →
      result.val.outcome ≠ Kernel.ExecutionOutcome.timeout
  stable : ∀ sec extra,
    RandCosted.valueDist
        (Kernel.runCosted cost (network sec) (fuel sec + extra) (initial sec)) =
      RandCosted.valueDist
        (Kernel.runCosted cost (network sec) (fuel sec) (initial sec))

namespace FuelCertificate

variable {cost : KernelCost M Address}
variable {network : (sec : CryptoLib.Core.SecPar) → NetworkAdapter family sec}
variable {initial : (sec : CryptoLib.Core.SecPar) → Configuration family policy sec}
variable {fuel larger : CryptoLib.Core.SecPar → Nat}
...
noncomputable def extend
    (certificate : FuelCertificate (policy := policy) cost network initial fuel)
    (fuel_le : ∀ sec, fuel sec ≤ larger sec) :
    FuelCertificate (policy := policy) cost network initial larger where
  noTimeout := by
    ...
    have value_mem_larger : result.val ∈
        (RandCosted.valueDist
          (Kernel.runCosted cost (network sec) (larger sec) (initial sec))).support := by
      rw [RandCosted.valueDist, PMF.mem_support_map_iff]
      exact ⟨result, hresult, rfl⟩
    have value_mem_base : result.val ∈
        (RandCosted.valueDist
          (Kernel.runCosted cost (network sec) (fuel sec) (initial sec))).support := by
  ...
  stable := by
    ...
    calc
      RandCosted.valueDist
          (Kernel.runCosted cost (network sec) (larger sec + extra) (initial sec)) =
        RandCosted.valueDist
          (Kernel.runCosted cost (network sec) (fuel sec) (initial sec)) := by
            simpa only [← fuel_add_difference, Nat.add_assoc] using
              certificate.stable sec (difference + extra)
      _ = RandCosted.valueDist
          (Kernel.runCosted cost (network sec) (larger sec) (initial sec)) := by
...
structure PPTExecutionCertificate
    ...
    (cost : KernelCost M Address)
    (network : (sec : CryptoLib.Core.SecPar) → NetworkAdapter family sec)
    (initial : (sec : CryptoLib.Core.SecPar) → Configuration family policy sec) where
  step : StepCostCertificate cost network
  activationLimit : CryptoLib.Core.SecPar → Nat
  stepRuntime : CryptoLib.Core.SecPar → Nat
  ...
  admission : PPTAdmissible M measure
    ...
    (fun sec configuration =>
      Kernel.runCosted cost (network sec) (activationLimit sec) configuration)
    (fun sec => activationLimit sec * stepRuntime sec)
  fuel : FuelCertificate (policy := policy) cost network initial activationLimit
...
def runtime
    (certificate : PPTExecutionCertificate (policy := policy) measure cost network initial)
...
noncomputable def toProbabilisticMachine
  ...
  run := fun sec configuration =>
    Kernel.runCosted cost (network sec)

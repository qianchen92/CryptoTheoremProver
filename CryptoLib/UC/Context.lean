...
structure KernelSimulation
    (addressMap : AddressRenaming Address Address)
    (outerCost : KernelCost M Address)
    ...
    (innerCost : KernelCost M Address)
  ...
  step_commutes : ∀ sec configuration,
    PMF.map
        ...
        (RandCosted.valueDist
          (Kernel.stepOne outerCost (outerNetwork sec) configuration)) =
      RandCosted.valueDist
        (Kernel.stepOne innerCost (innerNetwork sec)
...
variable
  {addressMap : AddressRenaming Address Address}
  {outerCost innerCost : KernelCost M Address}
  {outerNetwork : (sec : CryptoLib.Core.SecPar) → NetworkAdapter outerFamily sec}
  {innerNetwork : (sec : CryptoLib.Core.SecPar) → NetworkAdapter innerFamily sec}
  {outerInitial : (sec : CryptoLib.Core.SecPar) →
    Configuration outerFamily outerPolicy sec}
...
def mapStepResult
    (simulation : KernelSimulation addressMap outerCost outerNetwork
      outerInitial outerDecide innerCost innerNetwork innerInitial innerDecide)
...
theorem runCosted_commutes
    ...
    ∀ (fuel : Nat) (sec : CryptoLib.Core.SecPar)
      ...
      PMF.map (simulation.mapExecutionResult sec)
          (RandCosted.valueDist
            (Kernel.runCosted outerCost (outerNetwork sec) fuel configuration)) =
        RandCosted.valueDist
          (Kernel.runCosted innerCost (innerNetwork sec) fuel
      ...
      | none =>
                ...
                (RandCosted.valueDist
                  (RandCosted.bind
                    ...
                    (fun step =>
                      match step with
                      | .progressed updated =>
                          Kernel.runCosted outerCost (outerNetwork sec)
                            fuel updated
                      | .halted updated =>
              ...
              RandCosted.valueDist
                (RandCosted.bind
                  ...
                  (fun step =>
                    match step with
                    | .progressed updated =>
                        Kernel.runCosted innerCost (innerNetwork sec)
                          fuel updated
                    | .halted updated =>
                        match updated.output with
                        | some result => RandCosted.pure M
          ...
          let outerContinuation := fun step :
            ...
            RandCosted.valueDist <|
              match step with
              | .progressed updated =>
                  (Kernel.runCosted outerCost (outerNetwork sec) fuel updated)
              | .halted updated =>
                  match updated.output with
                  | some result => RandCosted.pure M
                      ({ outcome := Kernel.ExecutionOutcome.output result
          ...
          let innerContinuation := fun step :
            ...
            RandCosted.valueDist <|
              match step with
              | .progressed updated =>
                  (Kernel.runCosted innerCost (innerNetwork sec) fuel updated)
              | .halted updated =>
                  match updated.output with
                  | some result => RandCosted.pure M
                      ({ outcome := Kernel.ExecutionOutcome.output result
...
theorem decisionDist_commutes
    ...
    (fuel : Nat) (sec : CryptoLib.Core.SecPar)
    (configuration : Configuration outerFamily outerPolicy sec) :
    Kernel.decisionDist outerCost (outerNetwork sec) (outerDecide sec)
        fuel configuration =
      Kernel.decisionDist innerCost (innerNetwork sec) (innerDecide sec)
...
noncomputable def identity
    ...
    KernelSimulation (AddressRenaming.identity Address)
      cost network initial decide cost network initial decide where
...
noncomputable def comp
    ...
    {outerToMiddle middleToInner : AddressRenaming Address Address}
    {middleCost : KernelCost M Address}
    ...
    (outer : KernelSimulation outerToMiddle outerCost outerNetwork
      outerInitial outerDecide middleCost middleNetwork middleInitial
      middleDecide)
    (inner : KernelSimulation middleToInner middleCost middleNetwork
      middleInitial middleDecide innerCost innerNetwork innerInitial
      innerDecide) :
    KernelSimulation (outerToMiddle.comp middleToInner)
      outerCost outerNetwork outerInitial outerDecide
      innerCost innerNetwork innerInitial innerDecide where
  ...
  step_commutes := by
    ...
    let middleDist := RandCosted.valueDist
      (Kernel.stepOne middleCost (middleNetwork sec)
...
structure RealExecutionData
  ...
  certificate : PPTExecutionCertificate
    ...
    (policy := policy) measure kernelCost
    (fun sec => CryptoLib.UC.Network.adapter network
      (dispatchFamily environment.toEnvironment.machine protocol.machine
...
noncomputable def world
  ...
  composeReal environment.toEnvironment protocol adversary.toAdversary network
    policy data.kernelCost data.initial
...
noncomputable def world
  ...
  composeIdeal environment.toEnvironment functionality simulator.toSimulator
    network policy data.kernelCost data.initial
...
structure RealExecutionSimulation
  ...
  kernel : KernelSimulation
    ...
    addressMap outer.world.kernelCost outer.world.networkAdapter
      outer.world.initial outer.world.decision inner.world.kernelCost
...
noncomputable def identity
  ...
  realSimulation := by
    ...
    simpa only [WorldRenaming.identity_global] using (show
      ...
      kernel := KernelSimulation.identity certified.world.kernelCost

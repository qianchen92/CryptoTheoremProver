...
namespace CryptoLib.UC.Layered
...
abbrev SystemAddress (Tag : Type uAddress) (params : Parameters) :=
  CryptoLib.UC.Address Tag (Role params)
...
abbrev PartyAddress (Tag : Type uAddress) (params : Parameters) : Type uAddress :=
  CryptoLib.UC.Address Tag (PartyId params)
...
abbrev BoundaryAddress (Tag : Type uAddress) : Type uAddress :=
  CryptoLib.UC.Address Tag BoundaryRole
...
structure PartyStep
  ...
  State : CryptoLib.Core.SecPar → PartyAddress Tag params → Type uState
  Leakage : CryptoLib.Core.SecPar → PartyAddress Tag params → Type uLeakage
  Erasure : CryptoLib.Core.SecPar → PartyAddress Tag params → Type uErasure
  Output : CryptoLib.Core.SecPar → PartyAddress Tag params → Type uOutput
  ...
  realCertificate : ∀ adversary environment,
    PPTExecutionCertificate
      (family := dispatchFamily environment.toEnvironment.machine
        components.toAddressedITM adversary.toAdversary.machine network.machine)
      (policy := Corruption.layeredPolicy params) measure kernelCost

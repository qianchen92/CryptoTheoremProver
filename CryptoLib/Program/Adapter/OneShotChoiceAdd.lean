...
namespace CryptoLib.Program.Adapter.OneShotChoiceAdd
...
def prepareProgram
    (Parameter : Type uParameter) (Carrier : Type uCarrier) :
    Procedure (interpret Parameter Carrier) (Signature Parameter Carrier)
...
@[simp] theorem runCosted_prepare
    ...
    (adapter : Adapter M Parameter Carrier)
    (parameter : Parameter) (publicKey : Carrier) :
    Procedure.runCosted (algebra adapter)
  ...
  simp only [Procedure.runCosted, prepareProgram, Code.runCosted,
...
@[simp] theorem runCosted_reject
  ...
  letI := M.instAddMonoid
  simp only [Procedure.runCosted, rejectProgram, Code.runCosted,
    algebra_exec_tick, RandCosted.liftCosted_bind_liftCosted,
...
@[simp] theorem runCosted_query_used
  ...
  letI := M.instAddMonoid
  simp only [Procedure.runCosted, queryProgram, repeatQueryCode, Code.runCosted,
...
@[simp] theorem runCosted_query_fresh
  ...
  letI := M.instAddMonoid
  cases rightMessage <;>
    simp only [Procedure.runCosted, queryProgram, freshQueryCode, Code.runCosted,
...
theorem prepare_costBound
    ...
    {Parameter : Type uParameter} {Carrier : Type uCarrier}
    (adapter : Adapter M Parameter Carrier) :
    Procedure.CostBound (algebra adapter)
...
end CryptoLib.Program.Adapter.OneShotChoiceAdd

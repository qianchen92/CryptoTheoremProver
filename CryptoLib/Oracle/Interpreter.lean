import CryptoLib.Oracle.Handler
import CryptoLib.Oracle.Program
import CryptoLib.Oracle.Trace
...
noncomputable def runExact
    {α : Type (max uValue uResponse)} (program : Program issueCost α)
        ...
        PMF.bind (env.query name sec state oracleQuery) fun oracleResult =>
          PMF.pure
            ⟨ULift.up oracleResult.val.1, oracleResult.val.2,
              ...
              issueCost name oracleQuery, oracleResult.cost,
              issueCost name oracleQuery + oracleResult.cost⟩
...
@[simp] theorem runValueState_pure
    ...
    (value : α) (sec : CryptoLib.Core.SecPar)
    (env : OracleEnv.{uOracle, uQuery, uResponse, uState} Spec)
    (state : env.State) :
    runValueState (.pure value : Program issueCost α) sec env state =
...
@[simp] theorem runValueState_bind
    ...
    (first : Program issueCost α)
    (next : α → Program issueCost β)
    (sec : CryptoLib.Core.SecPar)
    (env : OracleEnv.{uOracle, uQuery, uResponse, uState} Spec)
    (state : env.State) :
...
@[simp] theorem runValueState_liftCosted
    ...
    (env : OracleEnv.{uOracle, uQuery, uResponse, uState} Spec)
    (state : env.State) :
    runValueState (.liftCosted dist : Program issueCost α) sec env state =
...
@[simp] theorem runValueState_query
    ...
    (env : OracleEnv.{uOracle, uQuery, uResponse, uState} Spec)
    (state : env.State) :
    runValueState (.query name query : Program issueCost
  ...
  simp only [runValueState, runExact, OracleEnv.zeroCost,
    PMF.map_bind, PMF.pure_map, RandCosted.sampleZeroCost,
...
theorem runValueState_eq_of_stateSimulation
    ...
    (querySimulation : ∀ (name : Spec.Name) (querySec : CryptoLib.Core.SecPar)
    ...
    {α : Type (max uValue uResponse)}
    (program : Program issueCost α)
    (sec : CryptoLib.Core.SecPar) (state : left.State) :

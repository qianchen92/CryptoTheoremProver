import CryptoLib.Oracle.Complexity.Implementation

namespace CryptoLib.Oracle.Complexity
...
structure OracleMachine
  ...
  issueCost :
    (sec : CryptoLib.Core.SecPar) → (input : Input sec) →
      (name : (Spec sec input).Name) → (Spec sec input).Query name → M.Cost
  program :
    (sec : CryptoLib.Core.SecPar) → (input : Input sec) →
      Oracle.Program (issueCost sec input)
...
end CryptoLib.Oracle.Complexity

...
namespace CryptoLib.Assumption.Program.DL.DDH
...
abbrev interpret
    (pp : CryptoLib.Assumption.DL.DDH.PublicParam.{uCost, uScalar, uGroup} M Scalar Carrier) :=
...
@[simp] theorem valueDist_exec_sampleScalar
  ...
  simpa [algebra, handler, CryptoLib.Program.Algebra.ScalarAction.algebra] using
    (CryptoLib.Assumption.DL.DDH.algebraLaws pp).exec_spec
...
end CryptoLib.Assumption.Program.DL.DDH
...
scoped macro:max "⦋" scalar:term "⦌" : term =>
  `($scalar •
    value(CryptoLib.Assumption.Program.DL.DDH.generatorValue))

...
structure Scheme where
  ...
  keygen :
    (pp : Param) →
      RandCosted M (PublicKey pp × SecretKey pp)
  encrypt :
    (pp : Param) →
    PublicKey pp →
    Message pp →
...
variable
    {M SecPar Param PublicKey SecretKey Message Ciphertext}
...
noncomputable def encryptDist
    ...
    PMF (Ciphertext pp) :=
  RandCosted.valueDist (E.encrypt pp publicKey message)
...
noncomputable def decryptDist
    ...
    PMF (Message pp) :=
  RandCosted.valueDist (E.decrypt pp secretKey ciphertext)

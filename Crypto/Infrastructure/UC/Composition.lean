...
def adapter
    ...
    (sec : Crypto.SecPar) : NetworkAdapter family sec where
...
def networkAdapter
    (world : RealWorld.{uCost, uAddress, uPayload, uPort, uCapability,
      uState, uLeakage, uErasure, uOutput}
      M EnvironmentAddress SystemAddress AdversarialAddress NetworkAddress schema)
    (sec : Crypto.SecPar) : NetworkAdapter world.family sec :=
  world.network.adapter world.family sec
...
def decision
    (world : RealWorld.{uCost, uAddress, uPayload, uPort, uCapability,
      uState, uLeakage, uErasure, uOutput}
      M EnvironmentAddress SystemAddress AdversarialAddress NetworkAddress schema)
    (sec : Crypto.SecPar) : MachineOutput world.family sec → Bool :=
  world.environment.closedWorldDecision world.protocol.machine
...
noncomputable def runCosted
    (world : RealWorld.{uCost, uAddress, uPayload, uPort, uCapability,
      uState, uLeakage, uErasure, uOutput}
      M EnvironmentAddress SystemAddress AdversarialAddress NetworkAddress schema)
    (sec : Crypto.SecPar) (activationFuel : Nat) :
    RandCosted M (Kernel.ExecutionResult world.family world.policy sec) :=
  Kernel.runCosted world.kernelAlgebra (world.networkAdapter sec)
...
def decision
    (world : IdealWorld.{uCost, uAddress, uPayload, uPort, uCapability,
      ...
      M EnvironmentAddress SystemAddress AdversarialAddress NetworkAddress schema)
    (sec : Crypto.SecPar) : MachineOutput world.family sec → Bool :=
  world.environment.closedWorldDecision world.functionality.machine

...
/-- Backwards-compatible randomized computation with natural-number path costs. -/
abbrev RandCosted (α : Type uValue) := RandCostedT natCostModel α

namespace RandCosted

noncomputable section

abbrev liftCosted {α : Type uValue} (result : Costed α) : RandCosted α :=
  RandCostedT.liftCosted result

abbrev pure {α : Type uValue} (value : α) : RandCosted α :=
  RandCostedT.pure natCostModel value

abbrev map {α : Type uValue} {β : Type uMapped}
    (f : α → β) (dist : RandCosted α) : RandCosted β :=
  RandCostedT.map f dist

abbrev bind {α : Type uValue} {β : Type uMapped}
    (dist : RandCosted α) (next : α → RandCosted β) : RandCosted β :=
  RandCostedT.bind dist next

abbrev sampleWithCost {α : Type uValue}
    (dist : PMF α) (cost : α → Cost) : RandCosted α :=
  RandCostedT.sampleWithCost dist cost

abbrev sampleZeroCost {α : Type uValue} (dist : PMF α) : RandCosted α :=
  RandCostedT.sampleZeroCost natCostModel dist

abbrev valueDist {α : Type uValue} (dist : RandCosted α) : PMF α :=
  RandCostedT.valueDist dist

abbrev costDist {α : Type uValue} (dist : RandCosted α) : PMF Cost :=
  RandCostedT.costDist dist

@[simp] theorem valueDist_pure {α : Type uValue} (value : α) :
    valueDist (pure value) = PMF.pure value :=
  RandCostedT.valueDist_pure natCostModel value

@[simp] theorem valueDist_liftCosted {α : Type uValue} (result : Costed α) :
    valueDist (liftCosted result) = PMF.pure result.val :=
  RandCostedT.valueDist_liftCosted result

@[simp] theorem valueDist_map {α : Type uValue} {β : Type uMapped}
    (f : α → β) (dist : RandCosted α) :
    valueDist (map f dist) = PMF.map f (valueDist dist) :=
  RandCostedT.valueDist_map f dist

@[simp] theorem valueDist_bind {α : Type uValue} {β : Type uMapped}
    (dist : RandCosted α) (next : α → RandCosted β) :
    valueDist (bind dist next) =
      PMF.bind (valueDist dist) fun value => valueDist (next value) :=
  RandCostedT.valueDist_bind dist next

@[simp] theorem valueDist_sampleWithCost {α : Type uValue}
    (dist : PMF α) (cost : α → Cost) :
    valueDist (sampleWithCost dist cost) = dist :=
  RandCostedT.valueDist_sampleWithCost (M := natCostModel) dist cost

@[simp] theorem costDist_sampleWithCost {α : Type uValue}
    (dist : PMF α) (cost : α → Cost) :
    costDist (sampleWithCost dist cost) = PMF.map cost dist :=
  RandCostedT.costDist_sampleWithCost (M := natCostModel) dist cost

@[simp] theorem valueDist_sampleZeroCost {α : Type uValue} (dist : PMF α) :
    valueDist (sampleZeroCost dist) = dist :=
  RandCostedT.valueDist_sampleZeroCost natCostModel dist

/-- Natural-number compatibility specialization of `RandCostedT.bind_cost_le`. -/
theorem bind_cost_le {α : Type uValue} {β : Type uMapped}
    ...
    ∀ result, result ∈ (bind first next).support →
      result.cost ≤ firstBudget + nextBudget :=
  RandCostedT.bind_cost_le first next firstBudget nextBudget first_cost_le next_cost_le

...
namespace Crypto.Infrastructure.Computation.Oracle.OracleEnv
...
noncomputable def withFixedSeed
    ...
    (initialState : State)
    (query : Seed → (name : Spec.Name) → Crypto.SecPar → State →
...
theorem runWithEnv_withDeferredSeed
  ...
  have querySimulation : ∀ (name : Spec.Name) (querySec : Crypto.SecPar)
  ...
  have fixedQuerySimulation : ∀ (name : Spec.Name)
      (querySec : Crypto.SecPar) (state : fixed.State)
...
noncomputable def withFixedOneShotSeed
    {Seed : Type uSeed}
    (answer : Seed → (name : Spec.Name) → Crypto.SecPar →
      Spec.Query name → PMF (Spec.Response name))
    (after : (name : Spec.Name) → Crypto.SecPar →
      Spec.Query name → PMF (Spec.Response name))
    (seed : Seed) : OracleEnv Spec where
  State := Bool
  init := false
...
theorem runWithEnv_withLazyOneShotSeed
  ...
  have deferredToLazy : ∀ (name : Spec.Name) (querySec : Crypto.SecPar)
  ...
  have deferredToMaterialized : ∀ (name : Spec.Name)
      (querySec : Crypto.SecPar) (state : deferred.State)
...
end Crypto.Infrastructure.Computation.Oracle.OracleEnv

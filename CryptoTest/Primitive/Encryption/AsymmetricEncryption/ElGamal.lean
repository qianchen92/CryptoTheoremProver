...
example
    ...
    CryptoFirstOrder.Program.runCosted
        ...
        (queryProgram Crypto.SecPar (ZMod 2) false)
...
example
    ...
    CryptoFirstOrder.Program.runCosted
        (algebra (reductionAdapter DDH.testFamily testReductionEfficiency))
        (queryProgram Crypto.SecPar (ZMod 2) true)
        (queryInputValue parameter right shared false leftMessage rightMessage) =
      RandCosted.liftCosted
...
def validReductionChallenge :
    Crypto.Assumption.DL.DDH.ChallengeInput DDH.testFamily where
  parameter := 7
  left := 0
  right := 1
  shared := 1
...
abbrev ReductionTraceCost :=
  CryptoTest.Infrastructure.Computation.TraceCost ReductionTraceEvent

abbrev reductionTraceCostModel :=
  CryptoTest.Infrastructure.Computation.TraceCost.costModel
    ReductionTraceEvent
...
def reductionTraceMeasure : NatMeasure reductionTraceCostModel :=
  CryptoTest.Infrastructure.Computation.TraceCost.lengthMeasure
    ReductionTraceEvent
...
example
    ...
    Nonempty
      (PPTMachine CostModel.nat NatMeasure.nat
        (fun _sec => Crypto.Assumption.DL.DDH.ChallengeInput DDH.testFamily)

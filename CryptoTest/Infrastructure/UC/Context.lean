...
namespace CryptoTest.Infrastructure.UC.Context
...
noncomputable def toyNetworkAdapter (_sec : Crypto.SecPar) :
...
noncomputable def innerInitial (sec : Crypto.SecPar) :
    Configuration toyFamily innerPolicy sec :=
  mapConfiguration (outerInitial sec)

example (sec : Crypto.SecPar) : (outerInitial sec).output = none := rfl
...
noncomputable def observingInitial (sec : Crypto.SecPar) :
...
noncomputable def environmentOwnedOutput (sec : Crypto.SecPar) :
    MachineOutput observingWorld.family sec :=
  ⟨.environment (), ULift.up true⟩

noncomputable def systemOwnedOutput (sec : Crypto.SecPar) :
    MachineOutput observingWorld.family sec :=
...
end CryptoTest.Infrastructure.UC.Context
